...
function init(owmlib) {
  ...
  owmlib.events.on("client", (client) => {
    if (client.workspace || client.ignoreWorkspace) return;
    let m = owmlib.monitors.monitorByOutput("default");
    if (!m) {
      m = owmlib.monitors.firstMonitor();
      if (!m) {
        logger.error("no monitor available for client", client.wmClass);
        return;
      }
      logger.info(
        "no 'default' monitor, using first monitor for client",
        client.wmClass,
      );
    }
